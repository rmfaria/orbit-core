...
import { getEpsSnapshot } from "./eps-tracker.js";
...
export function recordHttp(
  ...
  if (!httpMetrics.total.has(key) && httpMetrics.total.size >= MAX_HTTP_KEYS)
    return;

  httpMetrics.total.set(key, (httpMetrics.total.get(key) ?? 0) + 1);
  httpMetrics.durationCount.set(
    key,
    (httpMetrics.durationCount.get(key) ?? 0) + 1,
  );
  httpMetrics.durationSumMs.set(
    key,
    (httpMetrics.durationSumMs.get(key) ?? 0) + durationMs,
  );

  if (!httpMetrics.durationBuckets.has(key)) {
    httpMetrics.durationBuckets.set(
      key,
      Array(DURATION_BUCKETS_MS.length + 1).fill(0),
    );
  }
...
export function metricsPromHandler(_req: Request, res: Response) {
  ...
  lines.push(
    `pg_pool_total${labelsToStr({ pool: "api" })} ${pool ? pool.totalCount : 0}`,
  );
  lines.push(
    `pg_pool_total${labelsToStr({ pool: "worker" })} ${workerPool ? workerPool.totalCount : 0}`,
  );

  lines.push("# HELP pg_pool_idle Idle clients in the pool.");
  lines.push("# TYPE pg_pool_idle gauge");
  lines.push(
    `pg_pool_idle${labelsToStr({ pool: "api" })} ${pool ? pool.idleCount : 0}`,
  );
  lines.push(
    `pg_pool_idle${labelsToStr({ pool: "worker" })} ${workerPool ? workerPool.idleCount : 0}`,
  );

  lines.push("# HELP pg_pool_waiting Clients waiting for a pool connection.");
  lines.push("# TYPE pg_pool_waiting gauge");
  lines.push(
    `pg_pool_waiting${labelsToStr({ pool: "api" })} ${pool ? pool.waitingCount : 0}`,
  );
  lines.push(
    `pg_pool_waiting${labelsToStr({ pool: "worker" })} ${workerPool ? workerPool.waitingCount : 0}`,
  );

  // EPS (events per second)
  const eps = getEpsSnapshot();
  lines.push("# HELP orbit_ingest_eps Events per second (sliding window).");
  lines.push("# TYPE orbit_ingest_eps gauge");
  lines.push(
    `orbit_ingest_eps${labelsToStr({ source: "_global", window: "10s" })} ${eps.global.eps_10s}`,
  );
  lines.push(
    `orbit_ingest_eps${labelsToStr({ source: "_global", window: "1m" })} ${eps.global.eps_1m}`,
  );
  lines.push(
    `orbit_ingest_eps${labelsToStr({ source: "_global", window: "5m" })} ${eps.global.eps_5m}`,
  );
  for (const [src, v] of Object.entries(eps.sources)) {
    lines.push(
      `orbit_ingest_eps${labelsToStr({ source: src, window: "1m" })} ${v.eps_1m}`,
    );
  }

  lines.push("# HELP orbit_ingest_total Total ingested events since startup.");
  lines.push("# TYPE orbit_ingest_total counter");
  lines.push(
    `orbit_ingest_total${labelsToStr({ source: "_global" })} ${eps.global.total}`,
  );
  for (const [src, v] of Object.entries(eps.sources)) {
    lines.push(`orbit_ingest_total${labelsToStr({ source: src })} ${v.total}`);
  }
  ...
  for (const [key, v] of httpMetrics.total.entries()) {
    ...
    lines.push(
      `http_requests_total${labelsToStr({ method, route, status })} ${v}`,
    );
  }
  ...
  for (const [key, counts] of httpMetrics.durationBuckets.entries()) {
    ...
    lines.push(
      `http_request_duration_ms_sum${labelsToStr({ method, route, status })} ${sum.toFixed(3)}`,
    );
    lines.push(
      `http_request_duration_ms_count${labelsToStr({ method, route, status })} ${cnt}`,
    );
  }
