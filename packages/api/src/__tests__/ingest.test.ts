...
describe("ingestMetricsHandler", () => {
  ...
  it("returns 400 for invalid payload (missing metric)", async () => {
    ...
    const req = makeReq({
      metrics: [{ ts: "2024-01-15T10:30:00Z", asset_id: "a" }],
    });
    const res = makeRes();
...
describe("ingestEventsHandler", () => {
  ...
  it("accepts optional fingerprint and attributes", async () => {
    ...
    const req = makeReq({
      events: [
        { ...validEvent, fingerprint: "fp-001", attributes: { rule: "1234" } },
      ],
    });
    const res = makeRes();
    ...
    const insertCall = mockClient.query.mock.calls.find(
      (c: unknown[]) =>
        typeof c[0] === "string" &&
        (c[0] as string).toLowerCase().includes("insert into orbit_events"),
    );
    expect(insertCall).toBeDefined();
    const flatParams = (insertCall?.[1] as unknown[][]).flat();
    expect(flatParams).toContain("fp-001");
  });
});
